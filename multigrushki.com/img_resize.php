...
Отличительная особенность этой библиотечки является в том, что она умеет обраба-
тывать картинки формата BMP. На момент созднания этой функции GD не умела рабо-
тать с рисунками BMP, не путайте пожалуйста с WBMP.

Кто является создателем функции обработки BMP выяснить не удалось.
К остальным частям функции приложили свои руки многие программисты, в том числе
и я (webi.ru), улучшил и упростил использование.
...
// 0xFFFFF0 цвет фона(если рисунок получится меньше) в HEX-представлении
// 0 включить/выключить создание размера изображения, строго по размерам.
// Если включить, тогда размер изображения всегда будет таким, как заявлен,
// а лишнее будет заполняться фоном. Экспериментируйте.
...
$fon             -	указывает, если картинка получается меньше указанных размеров,
					тогда заполнить избыток фоном.
##############################################################################*/
function img_resize($src, $dest, $width, $height, $quality, $rgb, $fon)
{   // если не указан путь к исходному изображению, прерываем работу функции
	if (!file_exists($src)) return false;

    // если исходный файл не является изображением, прерываем работу функции
	...
	// если вдруг не пришла высота или ширина, прерываем работу функции
	if(!$width OR !$height) return false;

	// если не указано качество выходящего изображения, то ставим качество на 80%
    if (!$quality) $quality = 80;

    // если значение для качества выходящего изображения задано некорректно, то ставим качество на 80 или 100%
    else if ($quality < 1)	 $quality = 80;
	...
	*/
	...
	if($fon)
	{   // так создается картинка узаканного размера, а всё, где картинки нет,
		// заполнится фоном. Чтобы так создавать картинку, нижнюю строку надо
		// удалить, а с этой снять комментарии
